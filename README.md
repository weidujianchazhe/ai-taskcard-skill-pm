# 项目管理轻量化工作分配（Project Management Lite）
...
| 特性 | 说明 |
|---|---|
| **任务卡驱动接手** | 接手只读对应任务卡（描述/要点/涉及/锚点），精准到达，不漫读全项目 |
| **覆盖式状态** | 任务卡/STATE 永远覆盖为最新，不追加、不滚雪球；历史归 reports + INDEX |
| **收工四件套** | 交接 → 任务卡更新/删卡 → INDEX 登记 → STATE 同步，闭环不遗漏 |
| **双阅读层** | 人读区（3 行摘要）+ AI 工作坐标，人扫一眼即知任务定位 |
...
├── BLUEPRINT.md            # 设计蓝图指针
├── MAP.template.md         # 认路总图骨架（→ 实例化为 MAP.md）
...
└── tools\                 # 可选工具区（checks/visualize/schedule/report）
```
...
核心循环（运行期）：**读任务卡 → 干活 → 收工四件套（交接 → 卡更新/删卡 → INDEX → STATE）**

## 六、设计蓝图

完整设计蓝图见 `skill/项目管理轻量化工作分配蓝图.html`（含设计定稿、读取模型、验收清单）。
