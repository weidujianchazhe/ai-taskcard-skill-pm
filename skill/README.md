...
10. **稳定标识**：TASK-ID / EVENT-ID / DESIGN-ID 创建后永不变更；文件名可改，引用只用 ID；撞号必须重新生成并留痕
11. **设计卡生命周期**：设计卡状态机（草案 → 评审中 → 已批准 / 已拒绝 / 已废弃）+ 批准门禁——**未批准不得生成执行卡**；批准 / 拒绝 / 废弃均写 EVENT-ID
12. **三种协作模式**：light / standard / coordination——各自的文件集、角色与检查、升级路径；升级即扩集、历史不回写
13. **权限矩阵与单写者**：执行 AI / 审计 AI / 管理者 / 恢复 AI 四角色权限清晰；单写者责任 + 冲突文件 CONFLICT_*.md；写前读 revision/hash、优先原子 rename、CAS 兜底、写后验
14. **两处封顶装置**：INDEX 主文件行数与 STATE 字数上限——接手 AI 阅读成本的封顶装置，不是可扩容量；超限按覆盖式纪律把历史段移入 reports\\ 与归档
15. **边界声明**：本包只定义协议、模板与脚本**生成规范**；不实现或默认启用运行时守护进程、自动归档器、权限系统或持续监督
