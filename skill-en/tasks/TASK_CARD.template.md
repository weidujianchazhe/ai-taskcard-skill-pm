...
- **TASK-ID**: `TASK-YYYYMMDD-XXXXXX` (immutable; filename changes do not change this ID)
- **DESIGN-ID**: [approved design card ID, or `—`]
- **EVENT-ID**: [latest update event ID]
- **Revision**: `[MONOTONIC_REVISION]` · **RAW-SHA256**: `[HASH]` · **NORMALIZED-SHA256**: `[HASH]`
...
- **Concurrency metadata**: owner=[AI identity]; write=[atomic rename/CAS]; conflict=[path or `—`]
...
> **End-of-work protocol memo**: generate EVENT-ID and write the handoff → update/move this card → append the immutable archive index, then CAS-update INDEX → STATE entry-level write. Any failed completion-gate item blocks completion; only an approved DESIGN-ID may generate this execution card.
